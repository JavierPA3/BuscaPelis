...
def show_movie_id_by_name():
    ...
    if check_if_user_has_conection():
        return
    print("-----------------------------------------")
    while True:
        ...
        while True:
            continue_search = input("¿Desea avanzar hacia la siguiente página? Si/No: ")
            if continue_search.lower() != 'no' and continue_search.lower() != 'si':
                print("No es la respuesta que esperamos, tiene que responder Si/No. ")
            else:
                break
...
def print_five_trending_films():
    ...
    genre = input("Desea saber el trending de un género (inglés) en específico o de todos los géneros. (Género/Todos) ").title()
    id_genre = 0
    ...
    if genre != 'Todos':
        ...
        id_genre, user_wants_specific_genre = return_genre_id(genre, genre_data, id_genre, user_wants_specific_genre)
        # Si el género no existe el usuario volverá al menú.
        if not user_wants_specific_genre:
            print("Ese género no existe.")
            return

    type_week_or_day = return_day_or_week()
...
def return_day_or_week():
    while True:
        ...
        if day_or_week != 'SEMANA' and day_or_week != 'DIA':
            print("No es la respuesta que esperamos, tiene que responder Semana/Día. ")
        else:
            break
...
def check_if_user_has_conection():
    try:
        url = f'{URL_KEY}/movie/550?api_key={API_KEY}'
        requests.get(url)
    except ConnectionError:
        print("Error, no tiene internet.")
        return True
