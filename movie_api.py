...
def return_s_or_n():
    while True:
        continue_search = input("¿Desea avanzar hacia la siguiente página? S/N: ").lower()
        if continue_search == 's' or continue_search == 'n':
            return continue_search
        else:
            print("No es la respuesta qu esperamos, tiene que responder S/N. ")
    return continue_search
...
def print_five_trending_films():
    ...
    genre = input(
        "Desea saber el trending de un género (inglés) en específico o de todos los géneros. (Género/Todos) ").title()
    if check_if_user_has_conection():
        return
    id_genre = 0
